...
#define SENSOR_CLOSE_PDOWN
#ifdef SENSOR_CLOSE_PDOWN
#define SENSOR_FORNT_PDOWN RK29_PIN5_PD7
#define SENSOR_BACK_PDOWN RK29_PIN6_PB7 
#endif 
//#define camera_dv
...
#define SENSOR_INIT_PIXFMT V4L2_MBUS_FMT_YUYV8_2X8 //V4L2_PIX_FMT_YUYV
...
#ifdef camera_dv
static struct class *rk29_dv= NULL;

int rk29_camera_dv= 0;
#endif
struct reginfo
{
    u16 reg;
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (strcmp("gt2005_back", dev_name(icd->pdev)) == 0)
			{
			gpio_set_value(SENSOR_FORNT_PDOWN,0);
			
			}
		else
			{
			gpio_set_value(SENSOR_BACK_PDOWN,0);
...
//add by fjp======2011-09-30

    if (strcmp("gt2005_back", dev_name(icd->pdev)) != 0)
			{
			//sensor_write(client,0x0101,0x01);
...
static int sensor_deactivate(struct i2c_client *client)
{
   ...
   /* if (sensor->info_priv.funmodule_state & SENSOR_INIT_IS_OK)*/ {
		//printk("sensor_deactivate---------------------------------\n");
	  sensor_write(client, 0x100, 0x00);
	sensor_write(client, 0x102, 0x00);
	
	sensor_write(client, 0x104, 0x00);
    
#ifdef SENSOR_CLOSE_PDOWN
	
		if (strcmp("gt2005_back", dev_name(icd->pdev)) == 0)
				{
				gpio_set_value(SENSOR_FORNT_PDOWN,1);
				
				}
			else
				{
				gpio_set_value(SENSOR_BACK_PDOWN,1);
...
//	printk("800_600_ 2005SET  \n");
  ...
  //      	printk("1280_1024_ 2005SET  \n");
    ...
    else if (((set_w <= 1600) && (set_h <= 1200)) && sensor_uxga[0].reg)
    {
		...
		{
			                unsigned int  Shutter, AGain, DGain;
					char value;
					int  ret1;		
					...
					sensor_write(client,0x0300, 0xC1); //fix ae 


				       ret1 = sensor_read(client, 0x0012, &value);
				       Shutter |= (value << 8);	
				       ret1 = sensor_read(client, 0x0013, &value);
				       Shutter |= (value & 0xff);
					...
					if(Shutter<1)
				       ...
				       ret1 = sensor_read(client, 0x0014, &value);
				       AGain |= (value << 8);	
				       ret1 = sensor_read(client, 0x0015, &value);
				       AGain |= (value & 0xff);

				       ret1 = sensor_read(client, 0x0016, &value);
				       DGain |= (value << 8);	
				       ret1 = sensor_read(client, 0x0017, &value);
				       DGain |= (value & 0xff);
					   
					
					
					sensor_write(client,0x0304, Shutter >> 8);
					sensor_write(client,0x0305, Shutter);
					sensor_write(client,0x0306, AGain >> 8);
					sensor_write(client,0x0307, AGain);
					sensor_write(client,0x0308, DGain >> 2);
					sensor_write(client,0x0300, 0x01); // disable ae 
       		 }

//	    	printk("1600_1200_ 2005SET  \n");
...
#ifdef camera_dv
	if(((set_w==800)&&(set_h==600))||(rk29_camera_dv==1))
		{
//add by fjp======2011-09-30
...
//add by fjp======2011-09-30
    ...
    if (strcmp("gt2005_back", dev_name(icd->pdev)) != 0)
			{
			sensor_write(client,0x0101,0x00);
...
sensor_s_fmt_end:
	...
	if(svga_set==1)
		...
		msleep(500);
		sensor_write(client,0x0104, 0x03);
...
static int sensor_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *mf)
{
    ...
    {
    svga_set=1;
        winseqe_set_addr = sensor_svga;//sensor_vga;
        set_w = 640;
        set_h = 480;
		  //  printk("set_w ===%d,set_h ==%d\n",set_w,set_h);
    }
...
#ifdef camera_dv
static ssize_t rk29_camera_dv_show(struct class *cls, char *_buf)
{
	return sprintf(_buf, "%d\n", rk29_camera_dv);
}

static ssize_t rk29_camera_dv_store(struct class *cls, const char *_buf, size_t _count)
{
	rk29_camera_dv = simple_strtoul(_buf, NULL, 16);
	printk("rk29_camera_dv = %d\n",rk29_camera_dv);
	
	return _count;
} 

static CLASS_ATTR(rk29_camera_dv, 0666, rk29_camera_dv_show, rk29_camera_dv_store);
#endif
...
#ifdef camera_dv
	//=============add by fjp==============
	  rk29_dv = class_create(THIS_MODULE, "rk29_camera_dv");
	  class_create_file(rk29_dv,&class_attr_rk29_camera_dv);
	//==================================
#endif
