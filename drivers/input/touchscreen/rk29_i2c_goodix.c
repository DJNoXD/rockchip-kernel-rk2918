...
static int goodix_init_panel(struct rk_ts_data *ts)
{
	...
	uint8_t rd_cfg_buf[7] = {0x66,};
            ...
            uint8_t config_dpt_info[] = {
	0x65,0x01,0x04,0x00,0x03,0x00,0x0A,0x2E,
	0x1E,0xE7,0x32,0x05,0x08,0x10,0x4C,0x42,
	0x41,0x20,0x00,0x09,0x80,0x80,0x50,0x78,
	0x0E,0x0D,0x0C,0x0B,0x0A,0x09,0x08,0x07,
	0x06,0x05,0x04,0x03,0x02,0x01,0x00,0x1D,
	0x1C,0x1B,0x1A,0x19,0x18,0x17,0x16,0x15,
	0x14,0x13,0x12,0x11,0x10,0x0F,0x00,0x00,
	...
	0x00,0x00,0x00,0x00,0x01,0x00
	  ...
	  uint8_t config_duodian_info[] = { 
	0x65,0x01,0x04,0x00,0x03,0x00,0x0A,0x2E,0x1E,0xE7,0x32,0x05,0x08,0x10,0x4C,0x42,
	0x41,0x20,0x00,0x03,0x60,0x60,0x4B,0x64,0x0E,0x0D,0x0C,0x0B,0x0A,0x09,0x08,0x07,
	...
	//ret=goodix_i2c_write_bytes(ts->client,config_info, (sizeof(config_info)/sizeof(config_info[0])));
     if(gpio_get_value(RK29_PIN5_PA3)==GPIO_HIGH)
      	{
      	  
	   ret=goodix_i2c_write_bytes(ts->client,config_dpt_info, (sizeof(config_dpt_info)/sizeof(config_dpt_info[0])));
      	}
     else
      	{
	  ret=goodix_i2c_write_bytes(ts->client,config_duodian_info, (sizeof(config_duodian_info)/sizeof(config_duodian_info[0])));
      	}
	if (ret < 0) {
...
static void goodix_get_touch_info(struct rk_ts_data *ts,char *point_num,struct rk_touch_info* info_buf)
{
	...
	*point_num = touch_num;
	...
	{
	    // printk("track_id[%d] ==============  %d\n",index,track_id[index]);
	    // printk("info_buf[%d].status ==============  %d\n",index,info_buf[index].status);
...
//		goto exit;
    ...
    if(gpio_get_value(ts->irq_pin) == GPIO_LOW)
    {
         ...
         DBG("touch down .............\n");//add by fjp 2010-9-28
       	queue_delayed_work(ts->ts_wq, &ts->ts_work,msecs_to_jiffies(10));
	//	goto exit;
